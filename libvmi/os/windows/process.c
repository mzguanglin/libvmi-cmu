...
#define BLOCK_SIZE 1024 * 1024 * 1
    ...
    for (block_pa = 4096; block_pa + BLOCK_SIZE < vmi->size; block_pa += BLOCK_SIZE) { /*LibVMI bug fix: boundry
    						checking condition to prevent reading page out-of-bounds. -Guanglin*/
...
#define BLOCK_SIZE 1024 * 1024 * 1
    ...
    for (block_pa = start_address; block_pa + BLOCK_SIZE < vmi->size;
         block_pa += BLOCK_SIZE) {  /*LibVMI bug fix: boundry
    						checking condition to prevent reading page out-of-bounds. -Guanglin*/
        read = vmi_read_pa(vmi, block_pa, block_buffer, BLOCK_SIZE);
        if (BLOCK_SIZE != read) {
            continue;
